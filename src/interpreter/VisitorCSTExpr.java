...
import antlr.minipythonBaseVisitor;
...
public class VisitorCSTExpr extends minipythonBaseVisitor<Expr> {
    ...
    public Expr visitPrimary(minipythonParser.PrimaryContext ctx) {
        ...
        // check for string
        if (ctx.STRING() != null) {
            return new Expr.Literal(ctx.STRING().getText());
        }
        ...
        if (ctx.SUPER() != null) {
            ...
            Symbol methodSymbol = new Symbol(SymbolType.NAME, ctx.NAME().getText(), null, ctx.NAME().getSymbol().getLine(), ctx.NAME().getSymbol().getCharPositionInLine());
            return new Expr.Super(superSymbol, methodSymbol);
        }
        // check for grouping
        if (ctx.L_PAREN() != null) {
